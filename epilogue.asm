...
L_code_ptr_bin_apply:
        mov r14,qword[rsp+24] ;reserve closure
        mov r13,qword[rsp+32] ;reserve list pointer
        mov r12,0 ;argc counter
        mov r11,r13 ;cur list member
        assert_pair(r11)

        .s_list_argc_loop:
                cmp r11,sob_nil ; end of list
                je .s_list_argc_loop_end
                mov r11,SOB_PAIR_CDR(r11) ;move to next member
                inc r12 ; count member of the list
                jmp .s_list_argc_loop
        .s_list_argc_loop_end:
                cmp r12, 0 ; case 1
                je .s_zero_argv
                cmp r12, 1 ; case 2
                je .s_one_argv
                jmp .s_multi_argv ;case 3
        .s_zero_argv:
                mov r8,0 ;new argc
                mov qword[rsp+32],r8;shift&update argc to 0
                mov r8,SOB_CLOSURE_ENV(r14);get the f env.
                mov qword[rsp+24],r8;shift&update env(env of f)
                mov r8,qword[rsp]
                mov qword[rsp+16],r8;shift the current ret
                add rsp,16; fix rsp up by two
                jmp .s_complete ; go to closure code step
        .s_one_argv:
                mov r8,SOB_PAIR_CAR(r13);value of argv[0]
                mov qword[rsp+32],r8; place the argv[0]
                mov r8,1
                mov qword[rsp+24],r8;shift&update argc to 1
                mov r8,SOB_CLOSURE_ENV(r14);get the f env.
                mov qword[rsp+16],r8;shift&update env(env of f)
                mov r8,qword[rsp]
                mov qword[rsp+8],r8;shift the current ret
                add rsp,8; fix rsp up by one
                jmp .s_complete ; go to closure code step
        .s_multi_argv:
                mov r11,r12 ; list length = newargc
                sub r11,qword[rsp+16] ; old&new argc diff
                shl r11,3 ; offset in bytes
                mov r10,rsp ; original rsp
                sub r10, r11 ; new rsp address
                mov r15,r10 ; reserve new rsp
                mov r9,[rsp-8] ;
                mov [r10-8],r9 ;; reserve new rsp
                mov r9,[rsp]
                mov [r10],r9; shift frame - ret
                add r10,8 ;now r10 points to new env
                assert_closure(r14)
                mov r9,SOB_CLOSURE_ENV(r14)
                mov [r10],r9; shift frame - env(f)
                add r10,8 ;now r10 points to new argc
                mov [r10],r12; shift frame - new argc
                add r10,8 ; now r10 points to new argv[0]
                mov r11,r13 ;first list member
        .s_list_argv_loop:
                cmp r11,sob_nil ; end of list
                je .s_list_argv_loop_end
                mov r12,SOB_PAIR_CAR(r11)
                mov [r10],r12
                mov r11,SOB_PAIR_CDR(r11) ;move to next member
                add r10,8 ;new argv[i]->new argv[i+1]
                jmp .s_list_argv_loop
        .s_list_argv_loop_end:
                mov rsp,r15 ; new rsp address
        .s_complete:
                jmp SOB_CLOSURE_CODE(r14) ; call to f
...
fmt_char:
        db `%c\0`
fmt_param_count:
        db `Checking for arg value, found %ld\n\0`
fmt_arg_count_0:
...
fmt_division_by_zero:
        db `!!! Division by zero\n\0`
fmt_control_msg:
        db `We reached this part of code\n\0`
